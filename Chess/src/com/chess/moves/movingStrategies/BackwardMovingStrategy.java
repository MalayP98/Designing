...
public class BackwardMovingStrategy extends AbstractMovingStrategy {

    public BackwardMovingStrategy(BoardState boardState, int stepConstraint) {
    ...
    public List<MovableCoordinate> getMoves(MovableCoordinate currentCoordinate,
        ...
        for (int step = 1; step <= STEP_CONSTRAINT; step++) {
            nextCoordinate = (moves.isEmpty()) ? currentCoordinate.moveBackward()
                    : moves.get(moves.size() - 1).moveBackward();
