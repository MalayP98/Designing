...
public class NorthWestMovingStrategy extends AbstractMovingStrategy {

    public NorthWestMovingStrategy(BoardState boardState, int stepConstraint) {
    ...
    public List<MovableCoordinate> getMoves(MovableCoordinate currentCoordinate, PieceColor color) {
        ...
        for (int step = 1; step <= STEP_CONSTRAINT; step++) {
            nextCoordinate = (moves.isEmpty()) ? currentCoordinate.moveNorthWest()
                    : moves.get(moves.size() - 1).moveNorthWest();
